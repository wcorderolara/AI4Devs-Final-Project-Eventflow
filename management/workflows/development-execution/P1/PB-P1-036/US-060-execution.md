...
| Last Updated At | 2026-07-17T16:52:00Z |
| Completed At | 2026-07-17T16:52:00Z |
| Resume Session | 2026-07-17T16:45:00Z — reanudación para QA-002 IT + QA-006 concurrencia contra Postgres real (Docker `ef-eventflow` en puerto 5433). |
...
| TASK-PB-P1-036-US-060-QA-002 | Integration (atómica + regresión service) | 13 | BE-004, BE-006 | Done | 2026-07-17T16:45Z | 2026-07-17T16:52Z | AC-01..03, EC-01..05 | `tests/api/us060-create-booking-intent.integration.spec.ts` — 16 tests contra Postgres real (`describe.skipIf(!dbUp)`, mismo patrón US-056/057/058): AC-01 atomicidad (201 + Quote→accepted + BookingIntent→pending + 2 notifs `booking_intent.created`), AC-02 disclaimer ausente/false (400 con no-op DB), AC-03 5 campos de pago rechazados (payment_method, card_token, card_number, amount_paid, payment_intent_id), EC-01 valid_until vencida (410 QUOTE_EXPIRED), EC-02 status accepted/rejected (409 QUOTE_NOT_ACCEPTABLE), EC-03 doble POST (409 BOOKING_INTENT_ALREADY_EXISTS con details), EC-04 ajena/inexistente (404 uniforme), EC-05 UUID malformado (400 VALIDATION_ERROR), AUTH-TS-03 vendor 403, AUTH-TS-05 anon 401, TS-05 regresión service común (`POST /quotes/:id/reject` sigue emitiendo 2 notifs `quote.rejected` tras la extensión a 6 eventos). Ejecución: `DATABASE_URL=... npx vitest run tests/api/us060-create-booking-intent.integration.spec.ts` — 16/16 Passed en 1.37s. Regresión unit backend consolidada: 1192/1192 Passed (128 files, 60 skipped) — cero regresión. |
...
| TASK-PB-P1-036-US-060-QA-006 | Concurrencia (UNIQUE parcial) | 17 | DB-002, BE-004 | Done | 2026-07-17T16:45Z | 2026-07-17T16:52Z | UNIQUE constraint | Test dedicado en `tests/api/us060-create-booking-intent.integration.spec.ts` (bloque "QA-006 UNIQUE parcial DB"): (a) primer POST desde la app crea `pending` BookingIntent (201); (b) segundo `prisma.bookingIntent.create` directo en BD con el mismo `quote_id` DEBE fallar con `P2002` (verificado con `PrismaClientKnownRequestError.code === 'P2002'`); (c) validación positiva de la parcialidad `WHERE status IN ('pending','confirmed_intent')`: tras cancelar el primero, un nuevo INSERT `pending` para la misma Quote pasa sin conflicto. El index `uq_booking_intents_active_per_quote` verificado físicamente en Postgres (`docker exec ef-eventflow psql -c "\di uq_booking*"`). Guard multicapa: (1) `SELECT FOR UPDATE` en el UC serializa 2 POST simultáneos y transforma el segundo en `409 BOOKING_INTENT_ALREADY_EXISTS` con `booking_intent_id`; (2) UNIQUE parcial DB como último tapón — catch `P2002` re-tipa la violación. |
...
Ninguna. Los deferrals anteriores fueron completados en la reanudación 2026-07-17T16:45Z:

- **~~DEFERRED-US-060-IT-CONCURRENCY~~** — Resuelto. Se añadió `tests/api/us060-create-booking-intent.integration.spec.ts` (16 tests contra Postgres real, patrón `describe.skipIf(!dbUp)`) cubriendo TS-01..05 + concurrencia UNIQUE + regresión service común. 16/16 Passed. Ver QA-002 y QA-006 en §5.
...
- **Backend IT US-060** (nuevo, contra Postgres real): `DATABASE_URL=postgresql://AdminEF:...@localhost:5433/eventflow?schema=public npx vitest run tests/api/us060-create-booking-intent.integration.spec.ts` — **16/16 Passed** en 1.37s. Cubre TS-01..05 + concurrencia UNIQUE + regresión service común (`quote.rejected` sigue emitiendo 2 notifs tras la extensión del type).
- Backend IT (adaptaciones al schema `created_by`): `us036-budget-item-mutations.spec.ts`, `us046-public-vendor.api.spec.ts`, `us056-cancel-quote-request.integration.spec.ts`, `us039-committed-sync.integration.spec.ts`, `us096-quote-booking.integration.spec.ts`, `us096-quote-booking-security.spec.ts` actualizados con `createdBy` en todos los `prisma.bookingIntent.create` raw y con el nuevo body snake_case + disclaimer. `describe.skipIf(!dbUp)` consistente con el patrón US-058.
- Backend suite consolidada (unit + IT US-060): `DATABASE_URL=... npx vitest run tests/unit/ tests/api/us060-` — **1192/1192 Passed** (128 files, 60 skipped).
- Migración Prisma aplicada a Postgres: `DATABASE_URL=... npx prisma migrate deploy` — `20260717160000_us060_booking_intents_created_by_and_active_unique` aplicada limpiamente. Índice verificado: `docker exec ef-eventflow psql -U AdminEF -d eventflow -c "\di uq_booking*"` muestra `uq_booking_intents_active_per_quote` presente.
...
- **DEV-07** — ~~Deferred IT US-060~~ Resuelto en la reanudación 2026-07-17T16:45Z: se añadió `tests/api/us060-create-booking-intent.integration.spec.ts` (16 tests contra Postgres real). Migración aplicada, índice `uq_booking_intents_active_per_quote` verificado físicamente, 16/16 Passed.
- **DEV-08** — Ajuste de contrato de error: `EC-01 Quote vencida` mapea a HTTP **410 QUOTE_EXPIRED** (no 409). El Tech Spec §7 US-060 lista "409" por convención pero el catálogo `error-codes.ts` mantiene 410 (Gone) desde US-096 por consistencia con el endpoint legacy `POST /quotes/:id/accept`. El IT y los mensajes i18n reflejan el status real 410.

## 9. Final Result

- Resultado global: `DONE` — 18/18 tareas completadas.
- Backend: **1192/1192 tests verdes** (1176 unit + 16 IT US-060); 0 errores typecheck/lint. Frontend: **493/493 tests verdes**; 0 errores typecheck.
- OpenAPI actualizada + válida (44 paths).
- Documentación `docs/16 §32.6` + `docs/14 §10.9` actualizadas.
- Migración Prisma `20260717160000_us060_booking_intents_created_by_and_active_unique` aplicada limpiamente al Postgres local; migration.sql ready para `db:migrate:deploy` en CI/prod.
