...
describe.skipIf(!dbUp)('US-102 integración: constraints físicos', () => {
  ...
  describe('QA-002: rechazo de checks (SQLSTATE 23514)', () => {
    const cases: { name: string; sql: () => string }[] = [
      ...
      { name: 'chk_vendor_services_base_price_nonneg', sql: () => `INSERT INTO vendor_services (id,vendor_profile_id,service_category_id,title,price_min,updated_at) VALUES (gen_random_uuid(),'${ID.vprofile}','${ID.scat}','x',-1,now())` },
