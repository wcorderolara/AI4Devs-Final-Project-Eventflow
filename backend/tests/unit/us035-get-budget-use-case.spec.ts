...
describe('US-035 GetBudgetUseCase (BE-003, R1) — telemetría budget.viewed', () => {
  it('emite budget.viewed con enteros/montos y sin PII', async () => {
    ...
    let tick = 100;
    const uc = new GetBudgetUseCase(repo, new StaticCurrencyReadAdapter(), telemetry, () => {
      const now = tick;
      tick += 42;
      return now;
    });
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        itemsCount: 2,
        latencyMs: 42,
        // US-038 (BE-005): campos nuevos del snapshot.
        overcommittedAmount: 20,
        overCommittedItemsCount: 0,
      }),
    );
  });
});
