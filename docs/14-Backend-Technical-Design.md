...
- **Main use cases:** `CreateNotificationUseCase` (interno; llamado por otros módulos), `ListMyNotificationsUseCase` (US-071 surface organizer con `channel`/`status`/`page`/`pageSize`), `MarkNotificationAsReadUseCase` (US-072 single: ownership + no-revelación 404 + idempotencia), `MarkAllNotificationsAsReadUseCase` (US-072 bulk global con filtro `channel` opcional default `in_app` — aislamiento BR-NOTIF-005 forzado por `WHERE user_id = session.userId`).
...
- **Handlers in-tx (US-069 DOC-003):** `OnQuoteSentHandler` — invocado desde `RespondQuoteRequestUs052UseCase` (US-052 refactor BE-005) DENTRO de la misma `prisma.$transaction`. Reemplaza semánticamente la notif legacy `type='quote.sent'` (histórico US-049) por la canónica `type='quote_received'` dirigida al `event.owner_id`. Guards defensivos (quote no-`sent` / owner_id null / organizer suspended → warn + skip sin abortar la tx), idempotencia via `existsQuoteReceivedForQuote` sobre `payload->>'quoteId'`, resolución de idioma (`User.preferredLanguage → event.language → en`), doble INSERT (`type='quote_received'`, `channel='in_app'|'email_simulated'`) con `payload` sin PII (`{channel, languageCode, quoteId, quoteRequestId, eventId, vendorProfileId, title, body}` — nunca `totalPrice`, `breakdown`, `vendorName`) y log `event='email_simulated' template='notif.quoteReceived'`. Cualquier fallo del handler → rollback de la Quote (AC-06). Los templates i18n del subject/body son fijos por locale (sin interpolación) para prevenir fuga de PII.
- **Mutations mark-as-read (US-072 DOC-003):** `MarkNotificationAsReadUseCase` (single) + `MarkAllNotificationsAsReadUseCase` (bulk global). Ambos aplican aislamiento BR-NOTIF-005 con `WHERE user_id = session.userId` a nivel repositorio — el use case NUNCA acepta `userId` distinto al `actorUserId` derivado de la sesión. `MarkNotificationAsReadUseCase` implementa la política de no-revelación (`docs/19`): notif inexistente y notif ajena responden idénticamente con `404 RESOURCE_NOT_FOUND` (`NotFoundError` del shared kernel — evita enumeración de IDs privados). Idempotencia AC-06: si `read_at IS NOT NULL`, el use case cortocircuita antes del UPDATE y retorna 204 sin toque adicional. `MarkAllNotificationsAsReadUseCase` acepta `channel ∈ {in_app, email_simulated, all}` (default `in_app` D4 — paridad con US-071 D5); SQL bulk `UPDATE notifications SET read_at=now(), status='read' WHERE user_id=$1 AND status='unread' AND ($ch='all' OR payload->>'channel'=$ch)`. Emite log opcional `notif.markAllAsRead` con `{userId, channel, affected, correlationId}` sin PII (SEC-02). Endpoints: `PATCH /api/v1/notifications/:notificationId/read` y `POST /api/v1/notifications/mark-all-read?channel=<...>` — ambos responden `204 No Content` (D5).
- **Handlers in-tx (US-070 DOC-004):** `OnBookingConfirmedHandler` — invocado desde `ConfirmBookingIntentUseCase` (US-061 extendido BE-005) DENTRO de la misma `prisma.$transaction` como side-effect adicional al fan-out histórico `booking_intent.confirmed`. Emisión BILATERAL con `type='booking_confirmed'` canónico (organizer + vendor, 4 filas + 2 logs `[EMAIL]`). Guards globales D7 (status != `confirmed_intent` → skip global; `event.ownerId`/`vendor.userId` nulo → skip global). Dedup self-notification AC-08 (si `event.ownerId == vendor.userId` → 1 par con `recipientRole='organizer'` prioritario). Loop por recipient: guard `deactivated`/`suspended`/`null` → skip parcial (el otro sigue), idempotencia via `existsBookingConfirmedForRecipient` sobre `payload->>'bookingIntentId'` (aplicada 2× por rol), resolución de idioma independiente (fallback ladder D5), doble INSERT con `payload` sin PII (`{channel, languageCode, recipientRole, bookingIntentId, quoteId, quoteRequestId, eventId, vendorProfileId, title, body}` — nunca `totalPrice`/`breakdown`/`vendorName`) y log `event='email_simulated' template='notif.bookingConfirmed.<role>'`. Los templates i18n son FIJOS por locale × rol (sin placeholders — SEC-02). Cualquier fallo del handler → rollback del intent + `BudgetItem.committed` + ambos fan-outs juntos (AC-06). La firma `resolveMany(rows)` del resolver permanece retrocompatible: el dispatch por rol se apoya en `payload.recipientRole` persistido al INSERT.
- **Domain services / policies:** `NotificationRoutingService` (opcional; decide tipos y destinos), `BatchNotificationLinkResolver` (US-071/US-068/US-069/US-070 — genera `link` server-side por type con batch-lookup para evitar N+1; registro `LINK_STRATEGY_BY_TYPE` extendido en cada US propietaria; `booking_confirmed` dispatch por `payload.recipientRole` — organizer → `/organizer/events/{eventId}/bookings/{bookingIntentId}`, vendor → `/vendor/bookings/{bookingIntentId}` — batch-lookup via `NotificationLinkBookingIntentReader`).
- **Repository ports:** `NotificationRepository` (generic), `NotificationT7Repository` (US-034 T-7), `NotificationQrReceivedRepository` (US-068), `NotificationQuoteReceivedRepository` (US-069), `NotificationBookingConfirmedRepository` (US-070), `ListNotificationsRepository` (US-071), `MarkNotificationsRepository` (US-072 — `findOwnedById + markAsRead + markAllAsReadForUser`).
- **Adapters:** `NotificationSenderPort` con `InAppNotificationAdapter` (escribe Notification) y `SimulatedEmailAdapter` (escribe log estructurado). Adapters especializados por type: `LoggingSimulatedT7EmailAdapter` (US-034), `LoggingSimulatedQrReceivedEmailAdapter` (US-068), `LoggingSimulatedQuoteReceivedEmailAdapter` (US-069), `LoggingSimulatedBookingConfirmedEmailAdapter` (US-070).
- **Validaciones:** tipo ∈ enum de notificación (`task_due_soon`, `quote_request_received`, `quote_received`, `booking_confirmed`, …).
