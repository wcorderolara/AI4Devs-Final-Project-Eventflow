...
- **Domain services / policies:** `NotificationRoutingService` (opcional; decide tipos y destinos), `BatchNotificationLinkResolver` (US-071/US-068/US-069 — genera `link` server-side por type con batch-lookup para evitar N+1; registro `LINK_STRATEGY_BY_TYPE` extendido en cada US propietaria; `quote_received` → `/organizer/quote-requests/{quoteRequestId}/comparator`, reusa `NotificationLinkQuoteRequestReader` de US-068).
- **Repository ports:** `NotificationRepository` (generic), `NotificationT7Repository` (US-034 T-7), `NotificationQrReceivedRepository` (US-068), `NotificationQuoteReceivedRepository` (US-069), `ListNotificationsRepository` (US-071).
- **Adapters:** `NotificationSenderPort` con `InAppNotificationAdapter` (escribe Notification) y `SimulatedEmailAdapter` (escribe log estructurado). Adapters especializados por type: `LoggingSimulatedT7EmailAdapter` (US-034), `LoggingSimulatedQrReceivedEmailAdapter` (US-068), `LoggingSimulatedQuoteReceivedEmailAdapter` (US-069).
- **Validaciones:** tipo ∈ enum de notificación (`task_due_soon`, `quote_request_received`, `quote_received`, …).
