...
| **admin-governance** | Aprobaciones, moderación, métricas y auditoría | `AdminAction` | `ListPendingVendorsUseCase`, `ApproveVendorProfileUseCase`, `RejectVendorProfileUseCase`, `HideReviewUseCase`, `ListAdminMetricsUseCase`, `ListEventsForAdminUseCase` (US-078), `AdminViewEventUseCase` (US-016 + US-078 counts) | BR-ADMIN-001/002/009, `AdminAction` por cada acción de mutación (list de eventos NO emite AdminAction — Decisión PO US-078 D2), FR-EVENT-010 sólo-lectura arquitectónica en `admin/events` | `vendor-management`, `reviews-moderation`, `event-planning` (read-only), `notifications`, `shared-kernel` | Crear usuarios admin desde UI (solo seed/config); mutaciones admin sobre `Event` (FR-EVENT-010) | Sin "permissions UI" complejo. **US-078:** el submódulo `admin/events` expone únicamente 2 GETs (`/admin/events` list + `/admin/events/:id` detail); PATCH/DELETE/POST devuelven `403 FORBIDDEN_WRITE` (baseline US-016), preservando invariancia solo-lectura sin ADR nuevo. |
