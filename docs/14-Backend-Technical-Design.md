...
| **admin-governance** | Aprobaciones, moderación, métricas y auditoría | `AdminAction` | `ListPendingVendorsUseCase`, `ApproveVendorProfileUseCase`, `RejectVendorProfileUseCase`, `HideReviewUseCase`, `ListAdminMetricsUseCase`, `ListEventsForAdminUseCase` (US-078), `AdminViewEventUseCase` (US-016 + US-078 counts), `GetAdminMetricsUseCase` (US-079) | BR-ADMIN-001/002/009, `AdminAction` por cada acción de mutación (list de eventos NO emite AdminAction — Decisión PO US-078 D2), FR-EVENT-010 sólo-lectura arquitectónica en `admin/events` | `vendor-management`, `reviews-moderation`, `event-planning` (read-only), `notifications`, `shared-kernel` | Crear usuarios admin desde UI (solo seed/config); mutaciones admin sobre `Event` (FR-EVENT-010); métricas comerciales (SEC-02 US-079) | Sin "permissions UI" complejo. **US-078:** el submódulo `admin/events` expone únicamente 2 GETs (`/admin/events` list + `/admin/events/:id` detail); PATCH/DELETE/POST devuelven `403 FORBIDDEN_WRITE` (baseline US-016), preservando invariancia solo-lectura sin ADR nuevo. **US-079:** el submódulo `admin/metrics` expone `GET /admin/metrics` con cache in-memory `MetricsCacheService` TTL 60s (key `admin:metrics:v1`); 7 sub-queries agregadas ejecutadas en `Promise.all`. Sin AdminAction (Decisión PO D4). NO expone campos comerciales (SEC-02 / AC-05) — el DTO `AdminMetricsResponse` es la única fuente del contrato. |
