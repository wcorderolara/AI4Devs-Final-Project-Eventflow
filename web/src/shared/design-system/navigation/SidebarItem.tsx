...
const BASE =
  'group flex min-h-touch w-full items-center gap-3 rounded-button px-3 py-2 ' +
  'font-ui text-body-sm transition-colors duration-fast ease-standard';

export function SidebarItem({ item, className }: SidebarItemProps): React.JSX.Element {
  const { href, label, icon, active = false, badge, disabled = false, prefetch } = item;

  const content = (
    <>
      {/* Segunda señal del estado activo, además del fondo: barra vertical de acento. */}
      <span
        aria-hidden="true"
        className={cx(
          'h-5 w-0.5 shrink-0 rounded-badge',
          active ? 'bg-action-primary' : 'bg-transparent',
        )}
      />
  ...
  return (
    <Link
      ...
      className={cx(
        ...
        BASE,
        active
          ? 'bg-sidebar-item-active font-semibold text-sidebar-item-active'
          : 'text-primary hover:bg-sidebar-item-hover',
        className,
      )}
    >
      {content}
