...
// CategoryDeleteDialog (US-075 / FE-004). Confirmación de soft delete con reason
// obligatorio [10..500] (VR-10). Sin hard delete físico (SEC-04). Al éxito, invalida
// la lista admin y las queries públicas de categorías (US-042 + wizard vendor).
import { useCallback, useEffect, useId, useRef, useState, type FormEvent } from 'react';
import { useTranslations } from 'next-intl';
import { useDeleteCategory } from '../hooks/adminCategoriesQueries';
import type { AdminCategoryNode, AdminCategoryTreeNode } from '../api/adminCategoriesApi.types';
...
interface Props {
  node: AdminCategoryTreeNode | null;
  onClose: () => void;
  onSuccess?: (node: AdminCategoryNode) => void;
}

export function CategoryDeleteDialog({ node, onClose, onSuccess }: Props): React.JSX.Element | null {
  const t = useTranslations('admin.category.delete');
      ...
      <form method="dialog" onSubmit={onSubmit} className="flex w-[min(520px,90vw)] flex-col gap-4 p-6">
        ...
        <div>
          ...
          <textarea
            ...
            data-testid="category-delete-reason"
        ...
        <footer className="flex justify-end gap-2 pt-2">
          ...
          <button
            type="submit"
            disabled={!canSubmit}
            data-testid="category-delete-submit"
